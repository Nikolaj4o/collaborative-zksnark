...
impl<F: PrimeField> PoseidonParameters<F> {
    ...
    // share params
    pub fn share<MF: PrimeField + Reveal<Base = F>, R: Rng>(
        &self,
        rng: &mut R,
    ) -> PoseidonParameters<MF> {

        let shared_params = PoseidonParameters::<MF>::new(
            u32::king_share(self.full_rounds, rng),
            u32::king_share(self.partial_rounds, rng),
            u64::king_share(self.alpha, rng),
            Vec::<MF>::king_share_batch(self.mds.clone(), rng),
            Vec::<MF>::king_share_batch(self.ark.clone(), rng),
        );
        shared_params
    }
