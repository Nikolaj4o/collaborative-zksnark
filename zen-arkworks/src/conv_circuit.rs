        ...
        if num_channels * kernel_size * kernel_size < SIMD_BOTTLENECK {
                ...
                for n in 0..batch_size {
                    for h in 0..(input_height - kernel_size + 1) {
                        for w in 0..(input_width - kernel_size + 1) {
                            ...
                            let div_var =
                                FpVar::<Fq>::new_witness(ark_relations::ns!(cs, "div gadget"), || {
                                    Ok(div)
                                })
                                .unwrap();
            ...
            if fours >= 4 {
                ...
                for k in 0..num_kernels {
                    //println!("k {} multiplier {}", k, ((self.multiplier[k] * (2u32.pow(M_EXP)) as f32) as u128));
                    let multiplier: Fq =
                        ((self.multiplier[k] * (2u32.pow(M_EXP)) as f32) as u128).into();
                    let multiplier_var = FpVar::<Fq>::Constant(multiplier);
                    for n in 0..batch_size {
                        for h in 0..(input_height - kernel_size + 1) {
                            for w in (0..fours).step_by(4) {
                                ...
                                */
                                ...
                                let mut simd_extract_1 = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "shift result gadget1"),
                                    || Ok(Fq::zero()),
                                )
                                .unwrap();
                                let mut simd_extract_2 = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "shift result gadget2"),
                                    || Ok(Fq::zero()),
                                )
                                .unwrap();
                                let mut simd_extract_3 = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "shift result gadget3"),
                                    || Ok(Fq::zero()),
                                )
                                .unwrap();
                                let mut simd_extract_4 = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "shift result gadget4"),
                                    || Ok(Fq::zero()),
                                )
                                .unwrap();
                                ...
                                //println!("conv layer left {} == right {}", u32_res_debugging ,(self.y[n][k][h][w] - self.y_0) as u32 * 2u32u32.pow(24u32) + self.remainder[n][k][h][w]);
                                let div1: Fq = (self.div[n][k][h][w] as u64).into();
                                let div1_var = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "div1 gadget"),
                                    || Ok(div1),
                                )
                                .unwrap();
                                let remainder1: Fq = (self.remainder[n][k][h][w] as u64).into();
                                let remainder1_var = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "remainder1 gadget"),
                                    || Ok(remainder1),
                                )
                                .unwrap();
                            ...
                            for w in fours..(input_width - kernel_size + 1) {
                                ...
                                let div_var = FpVar::<Fq>::new_witness(
                                    ark_relations::ns!(cs, "div gadget"),
                                    || Ok(div),
                                )
                                .unwrap();
                                let remainder: Fq = (self.remainder[n][k][h][w] as u64).into();
                    ...
                    for h in 0..(input_height - kernel_size + 1) {
                        for w in 0..(input_width - kernel_size + 1) {
                            for k in (0..k_fours).step_by(4) {
                                ...
                                let multiplier2: Fq =
                                    ((self.multiplier[k + 1] * (2u32.pow(M_EXP)) as f32) as u128)
                                        .into();
                                let multiplier_var2 = FpVar::<Fq>::Constant(multiplier2);

                                let multiplier3: Fq =
                                    ((self.multiplier[k + 2] * (2u32.pow(M_EXP)) as f32) as u128)
                                        .into();
                                let multiplier_var3 = FpVar::<Fq>::Constant(multiplier3);

                                let multiplier4: Fq =
                                    ((self.multiplier[k + 3] * (2u32.pow(M_EXP)) as f32) as u128)
                                        .into();
