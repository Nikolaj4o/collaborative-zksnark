...
impl <F: PrimeField>ConstraintSynthesizer<F> for ArgmaxCircuitU8 {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        let _cir_number = cs.num_constraints();
        let argmax_fq: F = self.input[self.argmax_res].into();
        let argmax_var =
            FpVar::<F>::new_witness(ark_relations::ns!(cs, "argmax var"), || Ok(argmax_fq)).unwrap();

        for i in 0..self.input.len() {
            let tmp_fq: F = self.input[i].into();
            let tmp = FpVar::<F>::new_witness(ark_relations::ns!(cs, "argmax tmp var"), || Ok(tmp_fq))
                .unwrap();
