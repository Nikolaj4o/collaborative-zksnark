...
#[derive(Clone)]
pub struct LeNetCircuitU8OptimizedLv3Poseidon {
    pub params: SPNGParam,
...
impl ConstraintSynthesizer<Fq> for LeNetCircuitU8OptimizedLv3Poseidon {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
        // x commitment
        let flattened_x3d: Vec<Vec<Vec<u8>>> = self.x.clone().into_iter().flatten().collect();
        let flattened_x2d: Vec<Vec<u8>> = flattened_x3d.into_iter().flatten().collect();
...
#[derive(Clone)]
pub struct LeNetCircuitU8OptimizedLv3PolyClassification {
    pub x: Vec<Vec<Vec<Vec<u8>>>>,
...
impl ConstraintSynthesizer<Fq> for LeNetCircuitU8OptimizedLv3PolyClassification {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
        #[cfg(debug_assertion)]
        println!(
            "LeNetCircuitU8OptimizedLv3PedersenClassification is setup mode: {}",
...
#[derive(Clone)]
pub struct LeNetCircuitU8OptimizedLv3Poly {
    pub x: Vec<Vec<Vec<Vec<u8>>>>,
    pub conv1_weights: Vec<Vec<Vec<Vec<u8>>>>,
    pub conv2_weights: Vec<Vec<Vec<Vec<u8>>>>,
    pub conv3_weights: Vec<Vec<Vec<Vec<u8>>>>,
...
impl ConstraintSynthesizer<Fq> for LeNetCircuitU8OptimizedLv3Poly {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
        // x commitment
        let rho_var = FpVar::<Fq>::new_input(ark_relations::ns!(cs, "tmp"), || Ok(self.rho.clone())).unwrap();
        let beta_var = FpVar::<Fq>::new_input(ark_relations::ns!(cs, "tmp"), || Ok(self.beta.clone())).unwrap();
        let mut _cir_number = cs.num_constraints();
        let output: Vec<Vec<u8>> = lenet_circuit_forward_u8(
            self.x.clone(),
            self.conv1_weights.clone(),
