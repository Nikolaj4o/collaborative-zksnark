...
#[derive(Debug, Clone)]
pub struct FCCircuitOp3<F: PrimeField + std::convert::From<ark_ff::BigInteger256>> {
...
// constraints
// =============================
impl <F: PrimeField + std::convert::From<ark_ff::BigInteger256>>ConstraintSynthesizer<F> for FCCircuitOp3<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        if self.x.len() < 2u32.pow(SIMD_BOTTLENECK as u32) as usize {
            ...
            for i in 0..self.y.len() {
                ...
                let multiplier_fq: F = m.into();
                let multiplier_var = FpVar::<F>::Constant(multiplier_fq);
                ...
                let div1_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "div1 gadget"), || Ok(div1))
                        .unwrap();
            ...
            let bit_shift_1: F = (2u128.pow(16 + SIMD_3VEC_EXTRA_BITS)).into();
            let bit_shift_2: F = (2u128.pow((16 + SIMD_3VEC_EXTRA_BITS) * 3)).into();
            ...
            for i in (0..threes).step_by(3) {
                ...
                let filler1: F = garbage_filler.clone().into();
                garbage_filler.muln((16 + SIMD_3VEC_EXTRA_BITS) * 2);
                let filler2: F = garbage_filler.clone().into();
                garbage_filler.muln(16 + SIMD_3VEC_EXTRA_BITS);
                let filler3: F = garbage_filler.clone().into();
                garbage_filler.muln(16 + SIMD_3VEC_EXTRA_BITS);
                let filler4: F = garbage_filler.clone().into();
                ...
                */
                ...
                let mut simd_extract_1 =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "shift result gadget1"), || {
                        Ok(F::zero())
                    })
                    .unwrap();
                let mut simd_extract_2 =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "shift result gadget2"), || {
                        Ok(F::zero())
                    })
                    .unwrap();
                let mut simd_extract_3 =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "shift result gadget3"), || {
                        Ok(F::zero())
                    })
                    .unwrap();
                ...
                let one: F = b.into();
                let one_var = FpVar::<F>::Constant(one);
                let zero_var = FpVar::<F>::Constant(zero_fq);
                ...
                let multiplier1: F = m1.into();
                let multiplier_var1 = FpVar::<F>::Constant(multiplier1);

                let multiplier2: F = m2.into();
                let multiplier_var2 = FpVar::<F>::Constant(multiplier2);

                let multiplier3: F = m3.into();
                let multiplier_var3 = FpVar::<F>::Constant(multiplier3);
                ...
                let div2: F = (self.div[i + 1] as u64).into();
                let div2_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "div2 gadget"), || Ok(div2))
                        .unwrap();
                let remainder2: F = (self.remainder[i + 1] as u64).into();
                let remainder2_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "remainder2 gadget"), || {
                        Ok(remainder2)
                    })
                    .unwrap();
                ...
                let div3: F = (self.div[i + 2] as u64).into();
                let div3_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "div3 gadget"), || Ok(div3))
                        .unwrap();
                let remainder3: F = (self.remainder[i + 2] as u64).into();
                let remainder3_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "remainder3 gadget"), || {
                        Ok(remainder3)
                    })
                    .unwrap();
            ...
            let bit_shift_1: F = (2u128.pow(16 + SIMD_4VEC_EXTRA_BITS)).into();
            let bit_shift_2: F = (2u128.pow((16 + SIMD_4VEC_EXTRA_BITS) * 3)).into();
            let bit_shift_3: F = (2u128.pow((16 + SIMD_4VEC_EXTRA_BITS) * 4)).into();
            let bit_shift_1_const = FpVar::Constant(bit_shift_1);
            let bit_shift_2_const = FpVar::Constant(bit_shift_2);
            ...
            for i in (0..fours).step_by(4) {
                ...
                let filler4: F = garbage_filler.clone().into();
                garbage_filler.muln((16 + SIMD_4VEC_EXTRA_BITS) * 2);
                let filler5: F = garbage_filler.clone().into();
                ...
                */
                ...
                let mut simd_extract_4 =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "shift result gadget4"), || {
                        Ok(F::zero())
                    })
                    .unwrap();
                ...
                let multiplier4: F = m4.into();
                let multiplier_var4 = FpVar::<F>::Constant(multiplier4);
                ...
                let div4: F = (self.div[i + 3] as u64).into();
                let div4_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "div4 gadget"), || Ok(div4))
                        .unwrap();
                let remainder4: F = (self.remainder[i + 3] as u64).into();
                let remainder4_var =
                    FpVar::<F>::new_witness(ark_relations::ns!(cs, "remainder4 gadget"), || {
                        Ok(remainder4)
                    })
                    .unwrap();
...
fn scala_cs_helper_u8_simd<F: PrimeField>(
