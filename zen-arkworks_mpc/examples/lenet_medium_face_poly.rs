...
fn kzg_proof(
    ...
    let divisor_poly = DensePolynomial::<Fq>::from_coefficients_vec(vec![-x, Fq::from(1 as u64)]);
...
fn main() {
    ...
    //println!("LeNet optimized medium on ORL dataset");
    ...
    let person_feature_vector_f: Vec<Fq> = person_feature_vector.iter().map(|x| (*x).into()).collect();
    ...
    let flattened_z1d: Vec<Fq> = z.clone().into_iter().flatten().collect();
    ...
    cosine_similarity_f(z[0].clone(), person_feature_vector_f.clone(), Fq::from(50 as u8));
    ...
    let full_circuit = LeNetCircuitU8OptimizedLv3PolyRecognition{
        ...
        person_feature_vector: person_feature_vector_f.clone(),
        threshold: Fq::from(50 as u64),
